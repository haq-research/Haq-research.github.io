...
Models have the same three checks, and they are strictly pass or fail:
...
Capability isn't one number. It's at least seven skills that correlate far less than you'd expect: reasoning, domain knowledge, instruction following, coding, languages, modalities, and how well it fits a thinking mode workflow. A model can be a chess grandmaster and a terrible paralegal. So interview for the role.

Domain knowledge is the first thing to check. Models grew up reading different corpora. One might ace general trivia but never really read case law. General benchmarks are designed to average this away, so ask questions from your own field and score them.

Instruction following is the "brilliant but doesn't listen" hire. Genuinely smart models still drop a formatting rule or ignore a "never do X" when the instruction list gets long. In production, one ignored instruction breaks the pipeline. Measure it strictly: adherence should mean the share of outputs that honor every single instruction, not most of them. An answer that nails the content but breaks the format counts as a failure, because that's exactly how your parser will see it.

Thinking mode is the newest interview question. Should you pay extra for the candidate who reasons out loud? For hard math and gnarly code, often yes. For simple extraction, thinking mode is like hiring a philosopher to sort mail: slower, pricier, and occasionally worse. Decide with a ratio, not a vibe: it's only worth it when the quality you gain clears your bar against the extra cost and latency it costs you.
...
Watch what happens with real numbers. Candidate A costs $0.002 per call and is correct 60% of the time, but needs a human review on every output at $0.15 each. Work through the math and the true cost per good result is about $0.153. Candidate B costs $0.030 per call, fifteen times the sticker price, but is correct 95% of the time and needs no review at all. Its true cost per good result comes out to about $0.032.
...
Don't teach to the test. If you tune prompts against the same cases you score on, you'll ace your own eval and flunk production. Keep a held out set that prompt iteration never touches. This is the classic train and test leakage lesson, and it applies to prompt engineering exactly.
...
1. Gates: modality, rate limits, and compliance. Eliminate candidates before you ever score them.
2. Scores: capability vector, effective context, cost per successful task, p95 latency, and reliability rates. Measured on your own golden set, never a public benchmark.
3. Process: held-out integrity, failure mode analysis, routing and cascades, quarterly re-evaluation. This is what makes the numbers trustworthy and repeatable.
